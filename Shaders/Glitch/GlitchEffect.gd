...
func _ready():
	var _err = glitch_timer_node.connect("timeout",Callable(self,"_on_glitch_duration_timeout"))
	_err = sub_glitch_timer_node.connect("timeout",Callable(self,"_on_sub_glitch_duration_timeout"))
	_err = glitch_cool_down.connect("timeout",Callable(self,"_on_cooldown_timeout"))
...
func _generate_glitch():
	...
	var displace_amount = avg_displace + randf_range(0.0, displace_variance) * Math.rand_sign()
	var aberation_amount = avg_aberation + randf_range(0.0, aberation_variance) * Math.rand_sign()
	var sub_glitch_dur = avg_sub_glitch_duration + randf_range(0.0, sub_glitch_duration_variance) * Math.rand_sign()
	
	shader_material.set_shader_parameter("apply", true)
	shader_material.set_shader_parameter("displace_amount", displace_amount * Math.rand_sign())
	shader_material.set_shader_parameter("aberation_amount", aberation_amount)
...
func _on_glitch_duration_timeout():
	...
	var shader_material = target_texture.get_material()
	shader_material.set_shader_parameter("apply", false)
	
	if !one_shot:
		glitch_cool_down.set_wait_time(randf_range(avg_cool_down - cool_down_variance, avg_cool_down + cool_down_variance))
		glitch_cool_down.start()
...
func _on_cooldown_timeout():
	var dur = duration + randf_range(0.0, duration_variance) * Math.rand_sign()
