...
static func v2_rotate_deg(vec: Vector2, rotation: float) -> Vector2:
	return vec.rotated(deg_to_rad(rotation) * PI)


static func clamp_v(val: Vector2, min_val: Vector2, max_val: Vector2) -> Vector2:
	return Vector2(clamp(val.x, min_val.x, max_val.x), clamp(val.y, min_val.y, max_val.y))
...
static func compare_dir(dir1: Vector2, dir2: Vector2, max_angle_diff: float) -> bool:
	if dir1 in [Vector2(0.0, 0.0), Vector2(INF, INF)] or dir2 in [Vector2(0.0, 0.0), Vector2(INF, INF)]:
		return false
...
static func compute_triangle_surface(tri: PackedVector2Array) -> float:
	if tri.size() != 3:
		push_error("The given PackedVector2Array must contains exactly 3 vertices, current contains %d" % tri.size())
		return 0.0
...
static func compute_polygon_surface(polygon: PackedVector2Array) -> float:
	var sum : float = 0.0
...
static func rect_shape_to_polygon(rect_shape: RectangleShape2D, trans: Transform2D) -> PackedVector2Array:
	var local_coords = PackedVector2Array(
		[
			-rect_shape.extents,
...
static func find_most_top_left_v2(array: PackedVector2Array) -> Vector2:
	var top_left_v = Vector2.ONE * INF
	for v in array:
...
static func find_most_bottom_right_v2(array: PackedVector2Array) -> Vector2:
	var bottom_right_v = -Vector2.ONE * INF
	for v in array:
...
static func poly2rect(polygon: PackedVector2Array) -> Rect2:
	if polygon.size() != 4:
...
static func rect2poly(rect: Rect2) -> PackedVector2Array:
...
static func circle2poly(radius: float) -> PackedVector2Array:
...
static func capsule2poly(height: float, radius: float) -> PackedVector2Array:
...
static func shape2poly(shape: Shape2D, global_transform: Transform3D) -> PackedVector2Array:
	if shape is RectangleShape2D:
		...
		return xform(global_transform, PackedVector2Array([shape.a, shape.b]))
	
	return PackedVector2Array()


static func xform(transform: Transform2D, v2_array: PackedVector2Array) -> PackedVector2Array:
	var result_array = PackedVector2Array()
	
	for v in v2_array:
		result_array.append(transform * v)
	
	return result_array


static func circle_to_circle_intersection(p0: Vector2, r0: float, p1: Vector2, r1: float) -> PackedVector2Array:
...
static func compute_line_length(line : PackedVector2Array, to_id : int =  -1) -> float:
	var previous_point := Vector2.ONE * INF
	var total_dist : float = 0.0
	var max_id = line.size() if to_id == -1 else to_id + 1
	
	for i in range(max_id):
		var point = line[i]
		if previous_point != Vector2.ONE * INF:
...
static func find_line_nearest_point(point: Vector2, line: PackedVector2Array) -> Vector2:
	var nearest_point = Vector2.ONE * INF
