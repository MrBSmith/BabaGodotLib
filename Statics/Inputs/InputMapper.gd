...
var profiles_array: Array[InputProfile] = []

@export var print_logs: bool = false
@export var default_profile_id : PROFILES_PRESET = PROFILES_PRESET.KEYBOARD

@onready var current_profile_id : PROFILES_PRESET = default_profile_id:
	set(value): 
...
signal profile_inputs_map_changed(input_profile : InputProfile, player_id: int)
signal input_mapped(action_name, input_array)

class InputProfile extends Object:
	var dict := Dictionary()
...
func get_selected_profile() -> InputProfile: return profiles_array[current_profile_id]
func get_profile(id: int) -> InputProfile: return profiles_array[id]

func get_current_profile() -> InputProfile:
	return profiles_array[current_profile_id]
...
func map_input_profile(profile: InputProfile, player_id: int = -1) -> void:
	...
	profile_inputs_map_changed.emit(profile.dict, player_id)
...
func remap_action_key(action_name : String, input_array : Array):
	...
	input_mapped.emit(action_name, input_array)
