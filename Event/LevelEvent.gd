...
# TO DO: MOVE IT TO XION LEAK REPO
...
func is_class(value: String) -> bool: return value == "LevelEvent" or super.is_class(value)
...
func method_call():
	...
	if target_as_group:
		...
		if level.name == target_name:
			...
			var target = GAME.current_level.find_child(target_name)
			if target == null:
