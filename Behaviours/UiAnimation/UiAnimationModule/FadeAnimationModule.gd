...
@export_enum(FADE_MODE) var fade_mode : int = FADE_MODE.CONTENT
...
func is_class(value: String): return value == "FadeAnimationModule" or super.is_class(value)
...
func play() -> void:
	match(fade_mode):
		FADE_MODE.CONTENT:
			_fade_content()
			await self.content_fade_finished
		
		FADE_MODE.CONTAINER:
			_fade_container()
			await self.container_fade_finished
